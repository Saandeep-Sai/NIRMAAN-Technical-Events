...
export default function CodingDebugger() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await addDoc(collection(db, "coding_debugger_submissions"), {
                submittedBy: participants.trim(),
                answers: submissionAnswers,
                event: "Coding Debugger",
        ...
        <div className="page-container">
            ...
            <div className="animate-fade-in-up mb-12">
                <div className="flex items-center gap-3 mb-4">
                    <div className="event-badge" style={{ background: "rgba(99, 102, 241, 0.1)", border: "1px solid rgba(99, 102, 241, 0.2)", color: "#818cf8" }}>
                        🐛 Logic
                    </div>
                ...
                <h1 className="text-5xl sm:text-6xl font-black font-[family-name:var(--font-family-heading)] tracking-tighter mb-3 leading-[0.95]">
                    ...
                    <br />
                    <span className="text-[var(--color-text-primary)]">Debugger</span>
                </h1>
            ...
            <div className="animate-fade-in-up animate-delay-2 section-card p-7 mb-10">
                <h3 className="section-heading text-sm uppercase tracking-widest" style={{ color: "var(--color-accent-indigo)", marginBottom: "1rem", fontSize: "0.6875rem" }}>
                    How it works
                </h3>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    {[
                        { step: "01", icon: "🔍", text: "Read the buggy code carefully" },
                        { step: "02", icon: "✏️", text: "Write your corrected version below" },
                        { step: "03", icon: "🚀", text: "Submit all fixes together" },
                    ].map((item) => (
                        <div key={item.step} className="text-center p-4 rounded-xl" style={{ background: "rgba(12, 17, 38, 0.4)", border: "1px solid var(--color-border-subtle)" }}>
                            ...
                            <span className="text-[0.625rem] font-bold tracking-widest text-[var(--color-accent-purple)] block mb-1">
                                STEP {item.step}
                            </span>
            ...
            {fetchLoading ? (
                <div className="flex items-center justify-center py-24">
                    <div className="text-center">
                        ...
                        <p className="text-sm text-[var(--color-text-muted)] mt-5">Loading questions...</p>
                    </div>
                </div>
            ) : !released ? (
                <div className="section-card p-10 text-center animate-fade-in-up animate-pulse-glow">
                    <span className="text-4xl mb-4 block">⏳</span>
                    <p className="text-lg font-bold text-[var(--color-text-primary)] mb-2">Questions will be released soon</p>
                    <p className="text-sm text-[var(--color-text-muted)]">Please wait for the event coordinator to release questions.</p>
                </div>
            ) : questions.length === 0 ? (
                <div className="alert alert-warning text-center mb-12 animate-fade-in-up">
                    <p className="font-semibold">No questions have been released yet. Check back shortly.</p>
                </div>
            ) : (
                            ...
                            <div key={i} className={`animate-fade-in-up animate-delay-${Math.min(i + 1, 5)} section-card overflow-hidden`}>
                                ...
                                <div className="p-5" style={{ background: "rgba(8, 12, 30, 0.4)" }}>
                                    ...
                                    <textarea
                                        ...
                                        placeholder="Paste your corrected code for this question..."
                                        rows={6}
                                        className="input-field"
                                    />
                                </div>
